...
impl<'a, EXT, DB: Database> EvmHandler<'a, EXT, DB> {
    ...
    pub fn optimism<SPEC: Spec>(with_reward_beneficiary: bool) -> Self {
        ...
        handler.append_handler_register(HandleRegisters::Plain(if with_reward_beneficiary {
            crate::optimism::optimism_handle_register::<DB, EXT>
        } else {
            crate::optimism::optimism_handle_register_without_reward_beneficiary::<DB, EXT>
        }));
        handler
    }
